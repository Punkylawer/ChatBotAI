...
public class ArticlesRepository {
    ...
    private final Boolean shouldLoadArticles;

    public ArticlesRepository(
            @Value("${app.load-articles}") String shouldLoadArticles,
            EmbeddingStore<TextSegment> embeddingStore,
            EmbeddingModel embeddingModel) {
        this.embeddingStore = embeddingStore;
        this.embeddingModel = embeddingModel;
        this.shouldLoadArticles = Boolean.parseBoolean(shouldLoadArticles);

    }

    @PostConstruct
    public void init() throws IOException {
        if (Boolean.TRUE.equals(shouldLoadArticles)) {
            loadArticles();
        }
    }
    ...
    private List<TextSegment> splitIntoChunks(List<Document> documents, int maxTokensPerChunk, int overlapTokens) {
        DocumentByParagraphSplitter splitter = new DocumentByParagraphSplitter(maxTokensPerChunk, overlapTokens);
